...
const { AxiosThrottler } = require('axios-throttler')
AxiosThrottler.throttle(axios_1, AxiosThrottler.rps(5));
...
const getHttpStream = async (url, options = {}) => {
    const fetched = await axios_1.get(url.toString(), { ...options, responseType: 'stream' });
    return fetched.data;
};
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (const { hostname } of hosts) {
            ...
            let result;
            try {
                const body = await axios_1.post(url, stream, {
                    ...options,
                    headers: {
                        ...options.headers || {},
