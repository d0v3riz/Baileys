...
const pool = new StaticPool({
    size: 4,
    task: path.resolve(__dirname, 'encodeWorker.js')
});
...
const encodeBinaryNode = async ({ tag, attrs, content }, opts = constants, buffer = [0]) => {
    ...
    if (typeof content === 'string') {
        ...
        if (content.length > 1000) { // Threshold for batching
            const numWorkers = 4; // Number of worker threads in the pool
            const chunkSize = Math.ceil(content.length / numWorkers);
            const promises = [];
            for (let i = 0; i < numWorkers; i++) {
                const chunk = content.slice(i * chunkSize, (i + 1) * chunkSize);
                promises.push(pool.exec({ node: { tag, attrs, content: chunk }, options: opts }));
            }
            const results = await Promise.allSettled(promises);
            for (const result of results) {
                if (result.status === 'fulfilled') {
                    buffer.push(...result.value.result);
                } else {
...
module.exports = { encodeBinaryNode };